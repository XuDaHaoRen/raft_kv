...
enum ProgressState {
  ProgressStateProbe = 0, // 探测状态，当follower拒绝了最近的append消息时，那么就会进入探测状态
  ProgressStateReplicate = 1, // leader确认某个follower的同步状态
  ProgressStateSnapshot = 2 // 流量控制
};
...
// leader 用 progress 记录自己和 follower 的交互情况
class Progress {
 public:
  explicit Progress(uint64_t max_inflight)
      : match(0), // 保存目前为止，已复制给该follower的日志的最高索引值。如果leader对该follower上的日志情况一无所知的话，这个值被设为0。
        next(0), // 保存下一次leader发送append消息给该follower的日志索引，即下一次复制日志时，leader会从Next开始发送日志。
        state(ProgressState::ProgressStateProbe), // 当 follower 和 leader 日志不符时会进行这个回溯状态
        paused(false),
        pending_snapshot(0), // 因为 follower 拉下的太多所以直接发送 snap 
        recent_active(false),
