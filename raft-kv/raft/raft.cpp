...
Status Raft::step(proto::MessagePtr msg) {
  if (msg->term == 0) { // term 最起码从 1 开始

  } else if (msg->term > term_) { // 发起投票的阶段 
    if (msg->type == proto::MsgVote || msg->type == proto::MsgPreVote) { // 判断 msg 的状态是否在投票阶段
    ...
    switch (msg->type) {
      ...
      default:LOG_INFO("%lu [term: %lu] received a %s message with higher term from %lu [term: %lu]",
                       ...
                       msg->from,
                       msg->term);
        // 当收到一个 MsgApp 或者 MsgSnap 的消息后说明这个时候有人成功选举，那么其他的节点都变为 follower 
  ...
  } else if (msg->term < term_) { // 收到过去 term 的数据则直接拒绝请求，但是会根据 msg 的类型来决定 response 什么 msg
    if ((check_quorum_ || pre_vote_) && (msg->type == proto::MsgHeartbeat || msg->type == proto::MsgApp)) {
      ...
      /*
      如果收到一条 term 较小的 msg，说明这个 msg 在网络中延迟了，也有可能当前 node 有 advance 的term，现在既不能
      说明赢得选举，也不能重新加入旧的 term。如果 checkQuorum 是 false 则当前 node 发送 MsgVote 并附上自己的 term
      发送给 from ，如果是 true 则使用别的 Msg type 的数据进行发送
      但是如果 
      */
      //
  ...
  switch (msg->type) {
    case proto::MsgHup: { 
      if (state_ != RaftState::Leader) { // 当前没在 leader 状态
    ...
    default: {
      return step_(msg); // 根据不同的角色执行不同的 step 方法
    }
  }
...
Status Raft::step_follower(proto::MessagePtr msg) {
  switch (msg->type) {
    case proto::MsgProp:
      ...
      send(msg);
      break;
    case proto::MsgApp: { // 消息是 leader 发送来的
      election_elapsed_ = 0;
      lead_ = msg->from;
...
void Raft::handle_append_entries(proto::MessagePtr msg) {
  // 收到的 index 比发送的 index 要小说明网络延迟了，则返回当前 commit 的 index 
  ...
  // 将 msg 保存到 entry 中
  std::vector<proto::EntryPtr> entries; // 新创建的 entry
  ...
  // 将 msg->entry 添加到 unstable 中，并返回 last_index 和 ok
  raft_log_->maybe_append(msg->index, msg->log_term, msg->commit, std::move(entries), last_index, ok); 

  if (ok) { // 成功了想 from 返回自己 last_index 的数据
    proto::MessagePtr m(new proto::Message());
    m->to = msg->from;
    m->type = proto::MsgAppResp;
    m->index = last_index;
    send(std::move(m));
  } else { // 发生了错误并告诉 msg 应该返回的 last_index 
    uint64_t term = 0;
...
bool Raft::append_entry(const std::vector<proto::Entry>& entries) {
  uint64_t li = raft_log_->last_index(); // 得到 leader 最后的 index 
...
void Raft::reset_randomized_election_timeout() {
  randomized_election_timeout_ = election_timeout_ + random_device_.gen(); // 生成一个随机数
