...
void Unstable::maybe_term(uint64_t index, uint64_t& term, bool& ok) {
  ...
  if (index < offset_) { // 得到的 index 是在 snap 中的
    if (!snapshot_) {
      return;
    }
    if (snapshot_->metadata.index == index) { // 从 metadata.index 中得到 index 
  ...
  ok = true;
  term = entries_[index - offset_]->term; // 返回最后的 index 为 offset-index
}
