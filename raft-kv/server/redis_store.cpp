...
RedisStore::RedisStore(RaftNode* server, std::vector<uint8_t> snap, uint16_t port)
  ...
  if (!snap.empty()) {
    ...
    msgpack::object_handle oh = msgpack::unpack((const char*) snap.data(), snap.size()); // 反序列化
    try {
      oh.get().convert(kv); // 将 snap 的值反序列化在 kv 中
    } catch (std::exception& e) {
      LOG_WARN("invalid snapshot");
    }
    std::swap(kv, key_values_); // 将 kv 的值保存在另一个 map 中
  }
  // 初始化网络
  auto address = boost::asio::ip::address::from_string("0.0.0.0"); // 不知道本地的 ip ，检测所有本地 ip 
...
void RedisStore::start(std::promise<pthread_t>& promise) {
  ...
  worker_ = std::thread([this, &promise]() {
    // 这一步不知道为什么这么写
    promise.set_value(pthread_self()); // set_value : 共享数据的方法  pthread_self():获得线程自身 id 
