...
class AsioServer : public IoServer {
 public:
  explicit AsioServer(boost::asio::io_service& io_service,
    ...
    auto endpoint = boost::asio::ip::tcp::endpoint(address, port); // 服务端绑定 ip 地址和端口号

    acceptor_.open(endpoint.protocol());
    acceptor_.set_option(boost::asio::ip::tcp::acceptor::reuse_address(1)); // 设置端口复用
  ...
  void start() final {
    ...
    acceptor_.async_accept(session->socket, [this, session](const boost::system::error_code& error) {
      ...
      this->start();
      session->start_read_meta(); // async_read 
    });
  }
